...
const EthereumNews = () => {
    ...
    return(
        ...
        <div>
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight border-b-2 border-primary-600 my-4 mx-2">Ethereum News</h1>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 p-2">

                {data.news.articles.map((article) => { return(
                    <Link to={`/news/ethereum/${article.slug}`} key={article.id} className="flex flex-col items-center m-3 pt-2">
